...
"""
Model #6 — IL-ETransformer no_ifs  |  6 features  |  price + direction
=======================================================================
Qian (2025) exact replication — no IFS gate.

Features  : open, high, low, close, change_ratio, macd
Targets   : price     → next_close = close[t + h]
...
OUTPUT_SUMMARY     = RESULTS_DIR /"baseline_iltransformer_no_ifs_summary.csv"
OUTPUT_PREDICTIONS = RESULTS_DIR /"baseline_iltransformer_no_ifs_predictions.csv"
...
def norm_target(val: float, mean: np.ndarray, std: np.ndarray) -> float:
    """Normalize price target using close-feature stats (CLOSE_IDX=3)."""
    return (val - float(mean[CLOSE_IDX])) / float(std[CLOSE_IDX])


def denorm_target(val: float, mean: np.ndarray, std: np.ndarray) -> float:
    return val * float(std[CLOSE_IDX]) + float(mean[CLOSE_IDX])
...
class WarmupDataset(Dataset):
    def __init__(self, df: pd.DataFrame, seq_len: int,
        ...
        tgt_n  = np.array([norm_target(float(v), mean, std)
                           for v in df["price_target"].values], dtype=np.float32)
...
class ILETransformerNoIFS(nn.Module):
    ...
    def init_online(self, x_warmup: torch.Tensor,
        ...
        x_warmup : [B, L_w, 6]  normalized  — L_w can be arbitrarily large
        tf_warmup: [B, L_w, 3]

        FIX: Only the last seq_len steps are used for embedding + cache seeding.
        Passing the full L_w sequence to pos_enc caused a RuntimeError when
        L_w (e.g. 21,215) exceeded pos_enc.max_len (4,096). The FIFO caches
        hold exactly seq_len entries, so anything beyond the last seq_len rows
        carries no information forward into the online phase.
        """
        with torch.no_grad():
            # Seed causal conv buffer from full warmup tail (needs only CONV_PAD tokens)
            self.conv_embed.init_buf(x_warmup)

            # Slice to last seq_len steps — this is all the FIFO caches can hold,
            # and it keeps tensor length well within pos_enc.max_len (4096).
            x_seed  = x_warmup[:, -self.seq_len:, :]   # [B, seq_len, 6]
            tf_seed = tf_warmup[:, -self.seq_len:, :]  # [B, seq_len, 3]

            enc_emb = (self.conv_embed(x_seed)
                       + self.pos_enc(x_seed)
        ...
        self._online_pos = x_warmup.size(1)
        self.eval()
    ...
    def refresh_caches(self, x_hist: torch.Tensor,
        ...
        FIX: Slice to last seq_len steps before embedding to avoid exceeding
        pos_enc.max_len if the caller passes a longer history buffer.
        """
        with torch.no_grad():
            self.conv_embed.init_buf(x_hist)

            # Slice to last seq_len — consistent with init_online
            x_seed  = x_hist[:, -self.seq_len:, :]
            tf_seed = tf_hist[:, -self.seq_len:, :]

            enc_emb = (self.conv_embed(x_seed)
                       + self.pos_enc(x_seed)
                       + self.time_enc(tf_seed))
            x = enc_emb
            for layer in self.encoder:
...
def _seed_familiarity_buf(tsewc: TSEWC, warmup: pd.DataFrame,
    ...
    for i in range(start, len(warmup) - seq_len - 1):  # -1: target at i+seq_len must exist
        ...
        wt       = tf[i: i + seq_len]
        tgt_n    = norm_target(float(warmup["price_target"].iloc[i + seq_len]), m, s)
        tsewc.add_sample(
...
def evaluate_val_mae(model: ILETransformerNoIFS,
    ...
    def _infer_delay(frame: pd.DataFrame) -> int:
        ...
        tgt   = frame["price_target"].values.astype(np.float64)
        for h in TARGET_HORIZONS_BARS:
            if len(frame) <= h + 5:
                continue
            ok = np.isclose(tgt[:-h], close[h:], rtol=1e-7, atol=1e-5)
    ...
    for t in range(len(val)):
        ...
        # Buffer must hold full windows [1, seq_len, 6] for model.forward() in
        # Fisher estimation. h_arr[-seq_len:] is the window used for this prediction.
        ...
        delayed_updates.append({
            ...
            "x_buf": x_buf,
            "tf_buf": tf_buf,
            "target": float(val_tgt[t]),
        })

        realized_t = t - delay
        if realized_t >= 0:
            item    = delayed_updates[realized_t]
            true_n  = norm_target(item["target"], item["mean"], item["std"])
            ...
            if (t + 1) % EWC_UPDATE_FREQ == 0 and tsewc.is_novel(mse_err):
                ...
                tsewc.step(item["x_buf"].to(DEVICE), item["tf_buf"].to(DEVICE), y_t, online_opt, xh, tfh)
...
def evaluate_online(model: ILETransformerNoIFS,
    ...
    Direction: sign(pred_price − close[t]) — matches all other baselines.
    ...
    for t in range(len(online)):
        ...
        with torch.no_grad():
            pred_n = model.forward_online(x_new, tf_new).item()
        pred_price_all.append(denorm_target(pred_n, mean, std))
...
def compute_metrics(out: dict) -> dict:
    pred_p = out["pred_price"];  act_p = out["actual_price"]
    pred_d = out["pred_direction"]; act_d = out["actual_direction"]
    ...
    acc    = float(accuracy_score(act_d, pred_d))
    f1     = float(f1_score(act_d, pred_d, zero_division=0))
    prec   = float(precision_score(act_d, pred_d, zero_division=0))
    rec    = float(recall_score(act_d, pred_d, zero_division=0))
    return {
        "price_mae":              mae,
        "price_rmse":             rmse,
...
def main():
    ...
    print("\n" + "=" * 100)
    print("MODEL #6 — IL-ETransformer no_ifs  |  6 features  |  price + direction")
    print("=" * 100)
        ...
        for horizon in TARGET_HORIZONS_BARS:
            ...
            pred_df = online.iloc[LOOKBACK:].reset_index(drop=True)[
                ["symbol","datetime","open","high","low","close",
                 "change_ratio","macd","price_target","direction_target"]
            ].copy()
            pred_df["horizon"]           = horizon
            pred_df["model"]             = "iltransformer_no_ifs"
    ...
    if os.path.exists(OUTPUT_SUMMARY):
        ...
        cols    = ["symbol","horizon","model","price_mae","price_rmse",
                   "price_relative_mae_pct","direction_accuracy","direction_f1",
