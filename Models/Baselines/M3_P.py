...
"""
Offline LSTM Baseline  |  next_close (price) + direction targets
=================================================================
...
------------------------------------
seq_len      [16, 32, 64]
  Liu & Long (2020) used 10 past observations for daily data.
  For 1-minute data, 16-64 bars covers 16–64 minutes of recent history,
  sufficient to capture intraday microstructure patterns without excessive
  noise. Powers of 2 are preferred for GPU memory alignment.

hidden_size  [32, 64, 128]
  Liu & Long (2020) and Zhang et al. (2025) show that hidden unit count
  is among the most impactful LSTM hyperparameters. This range spans
  small (32), medium (64), and large (128) capacity, balancing expressiveness
  against overfitting risk on the limited warmup set.

num_layers   [1, 2]
  Greff et al. (2017) show that for financial time series, stacking beyond
  2 LSTM layers rarely improves performance and increases overfitting risk.
  A single layer is a strong baseline; 2 layers allows moderate depth.

dropout      [0.0, 0.1, 0.2, 0.4]
  Gal & Ghahramani (2016) formalise variational dropout for LSTMs.
  0.0 provides a no-regularisation reference. 0.1–0.2 are common light
  regularisation values. 0.4 is a stronger option for noisy financial data.

learning_rate [1e-4, 5e-4, 1e-3]
  Smith (2017) establishes this range as standard for Adam in deep learning.
  1e-3 is Adam's default; lower values allow finer convergence on volatile
  financial series.

batch_size   [64, 128]
  Keskar et al. (2017) show smaller batches generalise better. 64 and 128
  are the standard range for time-series tasks and fit comfortably in GPU
  memory.

epochs       [10, 20]
  Limited to prevent overfitting on the warmup set which is relatively small
  (20% of data). Zhang et al. (2025) use comparable epoch budgets for
  intraday LSTM baselines.

Random search — N_TRIALS=15:
  Bergstra & Bengio (2012) demonstrate that random search with as few as
  10–20 trials finds near-optimal configurations more efficiently than
  grid search for high-dimensional hyperparameter spaces. 15 trials balances
  search quality against computational cost.
...
OUTPUT_SUMMARY     = RESULTS_DIR /"baseline_lstm_offline_price_summary.csv"
OUTPUT_PREDICTIONS = RESULTS_DIR /"baseline_lstm_offline_price_predictions.csv"
OUTPUT_REMOVED_DAYS = RESULTS_DIR / "lstm_offline_removed_days.csv"
...
# =============================================================================
# NORMALIZATION
# Scaler is always fitted on warmup close prices only — no data leakage.
# =============================================================================

def fit_scaler(train_close: np.ndarray):
    mean = float(np.mean(train_close))
    std  = float(np.std(train_close)) + 1e-8
    return mean, std
...
class PriceSequenceDataset(Dataset):
    ...
    Target y[i]: price_target normalized, scalar
...
class LSTMModel(nn.Module):
    def __init__(self, hidden_size: int, num_layers: int, dropout: float):
        ...
        self.lstm = nn.LSTM(
            ...
            # dropout is applied between LSTM layers only when num_layers > 1
...
def evaluate_mae(
    ...
    """Validation MAE in original price scale — used for hyperparameter selection."""
    model.eval()
    preds, actuals = [], []
    with torch.no_grad():
        for X_batch, y_batch in loader:
            pred = model(X_batch.to(DEVICE)).cpu().numpy()
            preds.extend(denormalize(pred, mean, std))
            actuals.extend(denormalize(y_batch.numpy(), mean, std))
...
def run_trial(
    ...
    warmup_target: np.ndarray,
    val_close: np.ndarray,
    val_target: np.ndarray,
    mean: float,
    std: float,
) -> float:
    """Train on warmup, return validation MAE."""
    set_seed(RANDOM_SEED)

    seq_len = hp["seq_len"]

    train_ds = PriceSequenceDataset(warmup_close, warmup_target, seq_len, mean, std)

    # Prepend last seq_len warmup closes to val so first window has full context
    val_ds = PriceSequenceDataset(
        np.concatenate([warmup_close[-seq_len:], val_close]),
        np.concatenate([warmup_target[-seq_len:], val_target]),
        seq_len, mean, std,
    )
    ...
    return evaluate_mae(model, val_loader, mean, std)
...
def random_search(
    ...
    warmup_target = warmup["price_target"].values
    val_close     = val["close"].values
    val_target    = val["price_target"].values

    print(f"  [random_search] {n_trials} trials — Bergstra & Bengio (2012)")

    for t in range(n_trials):
        hp  = sample_hyperparams(rng)
        mae = run_trial(hp, warmup_close, warmup_target, val_close, val_target, mean, std)
...
def train_final_model(
    ...
    train_ds = PriceSequenceDataset(
        warmup_close, warmup_target, hp["seq_len"], mean, std
    )
...
def evaluate_offline(
    ...
    At each step t:
      - Input window = last seq_len closes from history
      - Predict next price
      - Append actual close[t] to history (walk-forward)

    Direction = 1 if predicted_price > current_close else 0
    """
    seq_len    = hp["seq_len"]
    ...
    # Seed history with full warmup to ensure window is always full
...
def compute_metrics(out: dict) -> dict:
    ...
    # Relative MAE as percentage of mean actual price
...
def main():
    ...
    print("\n" + "=" * 100)
    print("OFFLINE LSTM BASELINE  |  next_close (price) + direction  |  random search")
    print("=" * 100)
    ...
    for symbol in SYMBOLS:
        ...
        for horizon in TARGET_HORIZONS_BARS:
            ...
            # Scaler fitted on warmup only — prevents data leakage
            mean, std = fit_scaler(warmup["close"].values)

            # Hyperparameter selection via random search on validation set
            best_hp = random_search(warmup, val, mean, std, N_TRIALS)
