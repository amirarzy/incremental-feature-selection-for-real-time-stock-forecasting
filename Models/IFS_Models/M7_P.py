...
"""
Model #7 — IL-ETransformer + IFS  |  6 features  |  price + direction
=======================================================================
...
OUTPUT_SUMMARY      = RESULTS_DIR / "baseline_iltransformer_ifs_summary.csv"
OUTPUT_PREDICTIONS  = RESULTS_DIR / "baseline_iltransformer_ifs_predictions.csv"
OUTPUT_REMOVED_DAYS = RESULTS_DIR / "iltransformer_ifs_removed_days.csv"

GATE_DIR = RESULTS_DIR / "Gate_P"
GATE_DIR.mkdir(parents=True, exist_ok=True)

# Gate history written per (symbol, horizon) — see append_gate_history()
GATE_HISTORY_PREFIX = "gate_history"
...
class ILETransformerIFS(nn.Module):
    ...
    def init_online(self, x_warmup: torch.Tensor,
        ...
        FIX: Trim to last seq_len steps before embedding to avoid exceeding
        pos_enc.max_len (4096) when L_w (e.g. 21,215) is larger.
        IFS gate is applied to the trimmed slice; conv buffer seeded from
        the full warmup tail (needs only CONV_PAD tokens).
        """
        with torch.no_grad():
            # Trim to last seq_len — pos_enc and embedding expect L <= seq_len
            x_seed  = x_warmup[:, -self.seq_len:, :]   # [B, seq_len, 6]
            tf_seed = tf_warmup[:, -self.seq_len:, :]  # [B, seq_len, 3]

            # IFS gate applied to trimmed slice
            x_gated = self.ifs_gate(x_seed)             # [B, seq_len, 6]

            # Seed causal conv buffer from gated slice
            self.conv_embed.init_buf(x_gated)

            # Full-sequence embedding over trimmed window
            enc_emb = (self.conv_embed(x_gated)
                       + self.pos_enc(x_seed)
    ...
    def refresh_caches(self, x_hist: torch.Tensor,
        ...
        FIX: Trim to last seq_len steps before embedding.
        """
        with torch.no_grad():
...
def evaluate_online(model: ILETransformerIFS,
    ...
    if gate_records:
        ...
        print(f"  => gate history ({len(gate_df)} snapshots) saved to "
              f"{GATE_HISTORY_PREFIX}_{symbol}_h{horizon}.csv")
...
def main():
    ...
    print("\n" + "=" * 100)
    print("MODEL #7 — IL-ETransformer + IFS  |  6 features  |  price + direction")
    print("=" * 100)
    ...
    for symbol in SYMBOLS:
        ...
        for horizon in TARGET_HORIZONS_BARS:
            ...
            pred_df["model"]             = "iltransformer_ifs"
