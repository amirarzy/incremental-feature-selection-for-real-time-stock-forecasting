...
The goal of this repository is to make the full modelling pipeline clean, documented, reproducible, and executable by an external user.
...
Only complete Regular Trading Hours trading days are retained. A valid trading day must contain exactly 390 one-minute bars per symbol.
...
Close-only models use close-price history: M1, M2, M3, M4, and M5.
...
Full configuration details are provided in experiment_config.md.
...
The script executes all model files sequentially. If a model fails, the pipeline stops and reports the failed script.

Outputs

All generated outputs are written to Results/.

Execution logs are written to Results/logs/.
...
More details are documented in Results/README.md.
...
Place canonical_1m_rth_FULL.csv inside the Data/ folder.
Create a Python environment.
Install dependencies using pip install -r requirements.txt.
Run python3 run_all.py from the project root.
Inspect generated outputs in the Results/ folder.
Repository
https://github.com/amirarzy/incremental-feature-selection-for-real-time-stock-forecasting

The final Git commit hash should be recorded in experiment_config.md after the final push.

Notes

The dataset file is required for full reproduction.

The repository uses relative paths, so the code should not depend on server-specific absolute paths.
